...
public class AccountController {
	...
	public ResponseEntity<?> getAccountById(@PathVariable("aid") int aid){ 
	 ...
	 @GetMapping("/accountsByCust/{cust}")
	  public ResponseEntity<?> showByCustomer(@PathVariable String cust)  {
		 
		 List<Account> alist=accountService.getAccountsByCustomer(cust) ;
		 if(cust==null || cust.isEmpty()) {
			 return new ResponseEntity<String>("Enter Customer name",
					 					HttpStatus.INTERNAL_SERVER_ERROR);
		 }
		 else if (alist.isEmpty()){
		 return new ResponseEntity<String>("No Accounts by Customer name "+cust,
	 					HttpStatus.INTERNAL_SERVER_ERROR);
		 }
		 else
			 return new ResponseEntity<List<Account>>(alist,HttpStatus.OK);
	 ...
	 @GetMapping("/accountsByBalance/{minbal}/{maxbal}")
	  public ResponseEntity<?> showByBalanceRange(@PathVariable double minbal,
			  		@PathVariable double maxbal) {
		 List<Account> alist=accountService.getAccountsByBalanceRange(minbal, maxbal);
		 
		 if(minbal<0 || maxbal<0 || minbal>maxbal) {
			 return new ResponseEntity<String>("Enter proper minbal and maxbal",
	 					HttpStatus.INTERNAL_SERVER_ERROR);
		 }
		 else if (alist.isEmpty()){
			 return new ResponseEntity<String>("No Accounts by rangë",
		 					HttpStatus.INTERNAL_SERVER_ERROR);
			 }
		 else
			  return  new ResponseEntity<List<Account>>(alist,HttpStatus.OK);
